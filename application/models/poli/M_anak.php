...
class M_anak extends CI_Model
{
    ...
    public function get_pasien_poli_anak($cari = null)
    {
        $this->db->select('r.kode_invoice, r.nik, r.nama_pasien, r.nama_dokter, p.keluhan, p.id as id_pol_anak');
        $this->db->from('pol_anak p');
        $this->db->join('rsp_registrasi r', 'p.kode_invoice = r.kode_invoice', 'left');
        $this->db->where('r.nama_poli', 'Poli Anak');
        if ($cari) {
            $this->db->group_start();
    ...
    {
        $ada = $this->db->get_where('pol_anak', ['kode_invoice' => $kode_invoice])->row_array();
        if ($ada) {
            return $ada;
        } else {
            ...
            if ($pasien_data) {
                ...
                $this->db->insert('pol_anak', $data_to_insert);
                $new_id = $this->db->insert_id();
                return $this->db->get_where('pol_anak', ['id' => $new_id])->row_array();
            }
            return null;
        }
    }

    public function get_rekam_medis_detail($id_pol_anak)
    {
        $this->db->select('a.*, pas.*, a.id as id_pol_anak');
        $this->db->from('pol_anak a');
        $this->db->join('mst_pasien pas', 'a.id_pasien = pas.id');
        $this->db->where('a.id', $id_pol_anak);
        $data['rekam_medis'] = $this->db->get()->row_array();

        $data['diagnosa'] = $this->db->get_where('pol_anak_diagnosa', ['id_pol_anak' => $id_pol_anak])->result_array();
        $data['tindakan'] = $this->db->get_where('pol_anak_tindakan', ['id_pol_anak' => $id_pol_anak])->result_array();
    ...
    public function save_rekam_medis($post_data)
    {
        ...
        $id_pol_anak = $post_data['id_pol_anak'];
        $rekam_medis = $this->db->get_where('pol_anak', ['id' => $id_pol_anak])->row_array();
        $kode_invoice = $rekam_medis['kode_invoice'];

        $this->db->where('id', $id_pol_anak)->update('pol_anak', [
            'keluhan' => $post_data['keluhan'],
            'berat_badan' => $post_data['berat_badan'],
            'tinggi_badan' => $post_data['tinggi_badan'],
            'suhu' => $post_data['suhu'],
            'status_imunisasi' => $post_data['status_imunisasi'],
            'catatan' => $post_data['catatan']
        ]);

        $this->db->where('id_pol_anak', $id_pol_anak)->delete('pol_anak_diagnosa');
        if (isset($post_data['id_diagnosa'])) {
            $kelompok_diagnosa = [];
            foreach ($post_data['id_diagnosa'] as $key => $id) {
                $kelompok_diagnosa[] = ['id_pol_anak' => $id_pol_anak, 'id_diagnosa' => $id, 'diagnosa' => $post_data['diagnosa'][$key]];
            }
            if (!empty($kelompok_diagnosa))
                $this->db->insert_batch('pol_anak_diagnosa', $kelompok_diagnosa);
        }

        $this->db->where('id_pol_anak', $id_pol_anak)->delete('pol_anak_tindakan');
        ...
        if (isset($post_data['id_tindakan'])) {
            ...
            foreach ($post_data['id_tindakan'] as $key => $id) {
                ...
                $kelompok_tindakan[] = ['id_pol_anak' => $id_pol_anak, 'id_tindakan' => $id, 'tindakan' => $post_data['tindakan'][$key], 'harga' => $harga_tindakan];
                $total_biaya_tindakan += $harga_tindakan;
            }
            if (!empty($kelompok_tindakan))
                $this->db->insert_batch('pol_anak_tindakan', $kelompok_tindakan);
        }
        ...
        if ($id_pol_resep) {
            if (isset($post_data['resep_obat'])) {
                ...
                if (!empty($kelompok_resep))
                    $this->db->insert_batch('pol_resep_obat', $kelompok_resep);
            }
            ...
            if (isset($post_data['racikan'])) {
                foreach ($post_data['racikan'] as $racikan) {
                    ...
                    if (isset($racikan['bahan'])) {
                        ...
                        if (!empty($kelompok_bahan))
                            $this->db->insert_batch('pol_resep_racikan_detail', $kelompok_bahan);
                    }
                }
            }

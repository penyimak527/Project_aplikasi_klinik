...
class M_pengeluaran extends CI_Model
{
  public function result_data()
  {
    ...
    $sql = "SELECT a.* FROM rsp_pengeluaran a WHERE 1=1";
    $params = [];
  ...
  {
    $sql = $this->db->query("SELECT a.* FROM rsp_pengeluaran a WHERE a.id = ?", array($id));
  ...
  public function tambah()
  {
    ...
    $this->db->trans_begin();
    $this->db->insert('rsp_pengeluaran', $inputan);
  ...
  public function edit()
  {
    ...
    $this->db->update('rsp_pengeluaran', $inputan);
    $this->db->trans_complete();
    if ($this->db->trans_status() === FALSE) {
      $this->db->trans_rollback();
      $response = array(

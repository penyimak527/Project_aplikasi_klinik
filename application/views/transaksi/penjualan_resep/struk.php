    ...
    <div class="container">
        ...
        <div class="content">
            <table>
                ...
                <tr><td>Pasien</td><td class="text-right"><?php echo $data['pembayaran']['nama_customer']; ?></td></tr>
                <tr><td>Dokter</td><td class="text-right"><?php echo $data['pembayaran']['nama_dokter']; ?></td></tr>
            </table>
            <div class="divider"></div>
            <?php if (!empty($data['tindakan'])): ?>
                <strong>Tindakan:</strong>
                <table><?php foreach($data['tindakan'] as $item): ?><tr><td><?php echo $item['tindakan']; ?></td><td class="text-right">Rp <?php echo number_format($item['harga'], 0, ',', '.'); ?></td></tr><?php endforeach; ?></table>
            <?php endif; ?>
            <?php if (!empty($data['resep']) || !empty($data['racikan'])): ?>
                <div class="divider"></div>
                <strong>Obat:</strong>
                <table>
                    <?php foreach($data['resep'] as $item): $subtotal = ($item['harga'] + $item['laba']) * $item['jumlah']; ?>
                    <tr><td><?php echo $item['nama_barang'] . ' (x' . $item['jumlah'] . ')'; ?></td><td class="text-right">Rp <?php echo number_format($subtotal, 0, ',', '.'); ?></td></tr>
                    <?php endforeach; ?>
                    <?php foreach($data['racikan'] as $item): $subtotal = $item['sub_total_harga'] + $item['sub_total_laba']; ?>
                    <tr><td><?php echo $item['nama_racikan'] . ' (x' . $item['jumlah'] . ')'; ?></td><td class="text-right">Rp <?php echo number_format($subtotal, 0, ',', '.'); ?></td></tr>
                    <?php endforeach; ?>
                </table>
            <?php endif; ?>
            <div class="divider"></div>
            <table>
                ...
                <tr><td>Total Bayar</td><td class="text-right">Rp <?php echo number_format($data['pembayaran']['total_invoice'], 0, ',', '.'); ?></td></tr>
